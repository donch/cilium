...
if [ "$MODE" = "direct" ] || [ "$MODE" = "ipvlan" ] || [ "$MODE" = "routed" ] || [ "$NODE_PORT" = "true" ] ; then
	if [ "$NATIVE_DEV" == "<nil>" ]; then
		...
		bpf_load $NATIVE_DEV "$COPTS" "ingress" bpf_host.c bpf_host.o "from-netdev" $CALLS_MAP
		if [ "$NODE_PORT" = "true" ]; then
			bpf_load $NATIVE_DEV "$COPTS" "egress" bpf_host.c bpf_host.o "to-netdev" $CALLS_MAP
		else
...
bpf_load $HOST_DEV1 "$COPTS" "egress" bpf_host.c bpf_host.o from-netdev $CALLS_MAP
